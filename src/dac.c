...
uint32_t dacSampleTable[JBUF_FRAME_SIZE / 2 /* stereo short -> uint32_t */ * 2 /* double buffering */] = { 0 };
// { 0x0000, 0x1000, 0x2000, 0x3000, 0x4000, 0x5000, 0x6000, 0x7000, 0x8000, 0x9000, 0xA000, 0xB000, 0xC000, 0xD000, 0xE000, 0xF000 };
static void TIM6_Config(void);

uint16_t startup_ramp = 0;  // trying to reduce "pop" effect on startup - transition from DAC value 0 to DAC_OFFSET
                            // not efficient actually - seems like "pop" is just after powering DAC on
...
void TIM6_Config(void)
{
    ...
    htim.Init.Period = 90000000/SAMPLING_FREQUENCY - 1; //0x7F4; //roughly 44.1kHz (90000000/2047);
...
static void fill_dac_buffer(short* src, uint32_t *ptr)
{
    if (startup_ramp < DAC_OFFSET)
    {
        for (int i=0; i<JBUF_FRAME_SIZE/2; i++)
        {
            ptr[i] = startup_ramp;
            ptr[i] |= ((uint32_t)startup_ramp) << 16; // channel R
            if (startup_ramp < DAC_OFFSET)
            {
                if ((i & 0x07) == 0)
                {
    ...
    {
        int j=0;
        for (int i=0; i<JBUF_FRAME_SIZE; i+=2)
        {
            // left Scream channel: PA4, right channel: PA5
            ptr[j] = src[i] + DAC_OFFSET;
            ptr[j] |= ((uint32_t)src[i+1] + DAC_OFFSET) << 16; // channel R
            j++;
        }
    }
    else
    {
        ...
        {
            ptr[i] = DAC_OFFSET;
            ptr[i] |= ((uint32_t)DAC_OFFSET) << 16; // channel R
        }
    }
}
...
{
    //TRACE(("F\n"));
    uint32_t *ptr = dacSampleTable + ARRAY_SIZE(dacSampleTable)/2;
    short *src = jbuf_get();
    fill_dac_buffer(src, ptr);
}
